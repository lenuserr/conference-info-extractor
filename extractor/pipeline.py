...
from .llm import extract_basic, extract_details, DEFAULT_MODEL, DEFAULT_OLLAMA_URL
...
def extract_conference(
    ...
    Returns a dict with keys:
      ...
      - "meta": extraction metadata (model, attempts, pages_fetched)
    """
    # --- Step 1: Scrape ---
    ...
    if not site.pages:
        ...
        return {
            ...
            "meta": {"model": model, "attempts": 0, "pages_fetched": 0},
        }
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        attempts = attempt
        logger.info("Extraction attempt %d/%d with model=%s", attempt, MAX_RETRIES, model)

        # --- Step 2: LLM extraction (chain-of-extraction) ---
        basic = extract_basic(full_text, model=model, base_url=ollama_url)
        details = extract_details(full_text, model=model, base_url=ollama_url)
